...
export default function LiveDDU() {
  ...
  const currentLapMs = finite(frame?.captured_at) && lapStartedAt !== null
    ? Math.max(0, (frame.captured_at - lapStartedAt) * 1000)
    : null
  const deltaMs = currentLapMs !== null && finite(frame?.best_lap_ms) && frame.best_lap_ms > 0
    ? currentLapMs - frame.best_lap_ms
    : null
    ...
    <section className="ddu-grid">
      ...
      <aside className="systems-card panel">
        ...
        <div className="system-readings">
          ...
          <div className="boost"><span>BOOST</span><strong>{numberOrDash(frame?.boost, 2)}</strong></div>
          <div className="setup-reading"><span>⌁ TCS</span><strong>{numberOrDash(frame?.tcs)}</strong></div>
          <div className="setup-reading"><span>↔ BRAKE BIAS</span><strong>{finite(frame?.brake_bias) ? `${frame.brake_bias > 0 ? '+' : ''}${frame.brake_bias}` : '—'}</strong></div>
        </div>
      </aside>
    </section>
